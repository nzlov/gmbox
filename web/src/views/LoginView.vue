<template>
  <q-layout view="hHh lpR fFf">
    <q-page-container>
      <q-page class="row items-center justify-center gmbox-page">
        <q-card bordered class="full-width gmbox-dialog-compact">
          <q-card-section>
            <div class="text-overline text-primary">登录管理台</div>
            <div class="text-h5 text-weight-bold gmbox-top-gap-sm">欢迎回来</div>
            <div class="text-body2 text-grey-7 gmbox-top-gap-sm">
              输入管理员账号后进入控制台，继续处理邮箱同步、写信与账户维护。
            </div>

            <q-form class="column gmbox-stack-md gmbox-top-gap-lg" @submit.prevent="submit">
            ...
            <q-banner v-if="error" rounded dense class="gmbox-banner-error gmbox-top-gap-md">
              {{ error }}
            </q-banner>
